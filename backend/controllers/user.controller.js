...
export const register = async (req, res) => {
    try {
        // Debug: Log file and Cloudinary config
        ...
        if (!file) {
            return res.status(400).json({
                message: "Profile photo is required and was not received by the server.",
                success: false
            });
        }
...
export const updateProfile = async (req, res) => {
    try {
        ...
        // Only process file if it is present and is a buffer (uploaded file)
        if (file && file.buffer) {
